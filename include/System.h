...
public:
  ...
  void SaveTrajectoryEuRoC_append(const string &filename);
  ...
  double GetPixelstreamTS();
  uint8_t GetPixelstreamNum();
  string GetPixelstream();

  void SetRKTrackingMode(int mode);
  int GetRKTrackingMode();

  Sophus::SE3f GetTbc() { return settings_->Tbc(); }
  Sophus::SE3f GetTab() { return settings_->Tab(); }
