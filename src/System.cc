...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor, const bool bUseViewer, const int initFr, const string &strSequence)
    : mSensor(sensor), mpViewer(static_cast<Viewer *>(NULL)), mbReset(false), mbResetActiveMap(false), mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false),
      mbShutDown(false) {
  ...
  mpLocalMapper =
      new LocalMapping(this, mpAtlas, mSensor == MONOCULAR || mSensor == IMU_MONOCULAR, mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD, strSequence);
...
Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point> &vImuMeas, string filename) {
  ...
  if (settings_ && settings_->needToRectify()) {
    ...
    cv::remap(imLeft, imLeftToFeed, M1l, M2l, cv::INTER_LINEAR); //鱼眼图片矫正
...
void System::SaveTrajectoryEuRoC(const string &filename) {
  ...
  for (auto lit = mpTracker->mlRelativeFramePoses.begin(), lend = mpTracker->mlRelativeFramePoses.end(); lit != lend; lit++, lRit++, lT++, lbL++) {
    ...
    if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD) {
      ...
      Eigen::Quaternionf q = Twc.unit_quaternion();
      Eigen::Vector3f twc = Twc.translation();
      f << setprecision(6) << 1e9 * (*lT) << " " << setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w()
        << endl;
    }
...
// todo append some info after pose
void System::SaveTrajectoryEuRoC_append(const string &filename) {
  ...
  for (auto lit = mpTracker->mlRelativeFramePoses.begin(), lend = mpTracker->mlRelativeFramePoses.end(); lit != lend; lit++, lRit++, lT++, lbL++) {
    ...
    // cout << "4" << endl;
    // todo append some info
    int obsMps = pKF->GetNumberMPs();
    int trackedMps = pKF->TrackedMapPoints(1);
    ...
    if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD) {
      ...
      Eigen::Vector3f twb = Twb.translation();
      f << setprecision(6) << 1e9 * (*lT) << " " << setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w()
        << " obsMps: " << obsMps << " trackedMps: " << trackedMps << endl;
    } else {
      Sophus::SE3f Twc = ((*lit) * Trw).inverse();
      Eigen::Quaternionf q = Twc.unit_quaternion();
      Eigen::Vector3f twc = Twc.translation();
...
void System::SaveKeyFrameTrajectoryEuRoC(const string &filename) {
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD) {
      ...
      Eigen::Quaternionf q = Twb.unit_quaternion();
      Eigen::Vector3f twb = Twb.translation();
      f << setprecision(6) << 1e9 * pKF->mTimeStamp << " " << setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2) << " " << q.x() << " " << q.y() << " " << q.z() << " "
        << q.w() << endl;
...
void System::SaveTrajectoryKITTI(const string &filename) {
  ...
  for (list<Sophus::SE3f>::iterator lit = mpTracker->mlRelativeFramePoses.begin(), lend = mpTracker->mlRelativeFramePoses.end(); lit != lend; lit++, lRit++, lT++) {
    ...
    f << setprecision(9) << Rwc(0, 0) << " " << Rwc(0, 1) << " " << Rwc(0, 2) << " " << twc(0) << " " << Rwc(1, 0) << " " << Rwc(1, 1) << " " << Rwc(1, 2) << " " << twc(1) << " "
...
double System::GetPixelstreamTS() {
  if (mpFrameDrawer->mvtracked_time.size() > 0)
    return mpFrameDrawer->mvtracked_time.back();
  else
    return 0;
}
uint8_t System::GetPixelstreamNum() {
  if (mpFrameDrawer->mvtracked_num.size() > 0)
    return mpFrameDrawer->mvtracked_num.back();
  else
    return 0;
}
string System::GetPixelstream() {
  if (mpFrameDrawer->mvtracked_point.size() > 0)
    return mpFrameDrawer->mvtracked_point.back();
  else
    return "not_init";
}

void System::SetRKTrackingMode(int mode) { mpTracker->mRKTrackingMode = mode; }

int System::GetRKTrackingMode() { return mpTracker->mRKTrackingMode; }
